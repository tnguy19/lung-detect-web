...
for i in range(len(spike_times_ms)):
    if not clusters or spike_times_ms[i] - clusters[cluster_id][-1][1] > 5:  # Check if new cluster needed
        cluster_id += 1
...
for cluster in clusters.values():
    ...
    if len(channels_in_cluster) == num_channels:
...
for family_id, crackle_family in enumerate(crackle_families):
    ...
    # Extract 1-second slices around peaks for cross-correlation
    slice_length = int(sample_rate) / 2  # 1/2 second slice
    #slice is 1s slice filtered from audio data centered at mother peak
    mother_slice = audio_data[mother_channel, max(0, mother_sample - slice_length // 2):min(len(audio_data[mother_channel]), mother_sample + slice_length // 2)]
    ...
    for channel, time in crackle_family:
        ...
        daughter_slice = audio_data[channel, max(0, daughter_sample - slice_length // 2):min(len(audio_data[channel]), daughter_sample + slice_length // 2)]
